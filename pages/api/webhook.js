...
export default async function handler(req, res) {
  ...
  const p = b?.person || {};
  const phones = p?.telephone_numbers || {};
  const firstName = p.first_name || '';
  const lastName  = p.last_name  || '';
  const org       = p.organization || '';
  const phone     = phones.work || phones.mobile || phones.fax || '';
  const email     = p.email_address || '';
  const title     = p.title || '';
  const uuid      = b.uuid || '';
  const monthly   = parseFloat((b?.total?.recurring || '0').replace(/,/g,'')) || null;
  const upfront   = parseFloat((b?.total?.upfront   || '0').replace(/,/g,'')) || null;
  if (!phone && !email && !org) return res.status(200).json({ ok: true, skipped: 'no contact' });
  const lead = {
    firstName, lastName, org, phone, email, title,
    ...
    monthly, upfront, status: b.status || 'pending',
  };
